...
今天突然想知道C中函数是怎么在内存中分布的，虽然肯定是听过或看过到结论的，但是还是想自己亲手试试。
...
在main中，把三个实参的值存到了三个寄存器中，在sum中又从寄存器中读出，并拷贝到内存中间中。
...
我们知道，在操作系统中，高地址空间是内核代码，指令从高地址向低地址顺序排列。所以我们可以画出下面的图来表示当前的内存结构。
